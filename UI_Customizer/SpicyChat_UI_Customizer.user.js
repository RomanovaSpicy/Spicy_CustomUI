...
(function() {
    ...
    async function saveFontSettings() { try { await GM_setValue(STORAGE_FONT_SIZE_KEY, currentFontSize); } catch (e) { } }
    async function saveWidthSettings() { try { await GM_setValue(STORAGE_WIDTH_KEY, currentWidth); } catch (e) { } }
    async function saveTheme() { try { await GM_setValue(STORAGE_THEME_KEY, currentThemeKey); } catch (e) { } }
    async function saveBackgroundUrl() { try { await GM_setValue(STORAGE_BACKGROUND_URL_KEY, customBackgroundUrl); } catch (e) { } }
    async function saveOpacitySettings() { try { await GM_setValue(STORAGE_OPACITY_KEY, currentOpacity); } catch (e) { } }
    ...
    async function loadSettings() {
        ...
        } catch (e) { currentThemeKey = DEFAULT_THEME_KEY; currentFontSize = DEFAULT_FONT_SIZE; currentWidth = DEFAULT_WIDTH_SLIDER; customBackgroundUrl = DEFAULT_BACKGROUND_URL; currentOpacity = DEFAULT_OPACITY; }
    }
    ...
    async function initialize() {
        ...
        try {
            ...
            if (!settingsView || !themesView || !fontSlider || !fontValue || !widthSlider || !widthValue || !opacitySlider || !opacityValue || !themeDisplay || !changeThemeBtn || !resetBtn || !themeListContainer || !backBtn || !backgroundUrlInput || !applyBgBtn || !resetBgBtn) { throw new Error("GUI elements missing after creation!"); }
