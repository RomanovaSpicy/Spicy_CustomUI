...
(function() {
    ...
    const BACKGROUND_TARGET_SELECTOR = '[data-ui-component="ChatMessageScrollContainer"]';
    ...
    const guiStyles = `
        ...
        #${POPUP_ID} #${VIEW_SETTINGS_ID} .customizer-section:first-of-type { margin-top: 18px !important; }
        ...
        #${POPUP_ID} span[id*="-value"] { min-width: 40px; text-align: right; font-family: monospace; font-size: 14px; font-weight: bold; color: #eceff4; background: rgba(59, 66, 82, 0.4); padding: 2px 5px; border-radius: 4px; user-select: none; }
        ...
        if (customBackgroundUrl && customBackgroundUrl.trim() !== '') {
            try {
                const safeUrl = CSS.escape(customBackgroundUrl.trim());
                 ...
                 console.error("Error escaping background URL:", e);
    ...
    async function loadSettings() {
        try {
            ...
            if (!themes[currentThemeKey]) { currentThemeKey = DEFAULT_THEME_KEY; await saveTheme(); }
        } catch (e) { console.error("Error loading settings:", e); currentThemeKey = DEFAULT_THEME_KEY; currentFontSize = DEFAULT_FONT_SIZE; currentWidth = DEFAULT_WIDTH_SLIDER; customBackgroundUrl = DEFAULT_BACKGROUND_URL; }
    }
    ...
    function togglePopup() {
        if (!popupElement) { console.error("togglePopup: popupElement missing!"); return; }
    ...
    async function initialize() {
        ...
        if (!logoElement) { setTimeout(initialize, 1500); return; }
        isInitialized = true;
        console.log(`SC UI Customizer v${GM_info.script.version}: Initializing...`);
        ...
        try {
            ...
            if (!settingsView || !themesView || !fontSlider || !fontValue || !widthSlider || !widthValue || !themeDisplay || !changeThemeBtn || !resetBtn || !themeListContainer || !backBtn || !backgroundUrlInput || !applyBgBtn || !resetBgBtn) { throw new Error("GUI elements missing after creation!"); }
            ...
            console.log(`SC UI Customizer v${GM_info.script.version}: Initialization complete.`);
        } catch (error) { console.error("!!! Error during UI Customizer setup:", error); isInitialized = false; }
    }
