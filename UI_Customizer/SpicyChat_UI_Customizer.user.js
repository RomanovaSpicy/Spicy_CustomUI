...
(function() {
    ...
    // More specific target for observer: [data-ui-component="ChatMessageScrollContainer"] > div:first-child > div:first-child
    const CHAT_MESSAGES_CONTAINER_SELECTOR = '[data-ui-component="ChatMessageScrollContainer"] > div:first-child > div:first-child';
    ...
    const guiStyles = `
       ...
       [data-ui-component="ChatMessageScrollContainer"] { padding-bottom: 60px !important; }
    `;
    ...
    function processSingleAvatar(avatarButton, size) {
        ...
        if (img) {
            if (!img.dataset.originalSrcAttempted) {
                let originalSrc = img.src;
                let newSrc = originalSrc;
                const classParamPattern = /\?class=avatar\d+x\d+/;

                if (originalSrc && originalSrc.match(classParamPattern)) {
                    newSrc = originalSrc.replace(classParamPattern, '');
                } else if (originalSrc && originalSrc.includes('?class=')) {
                    // Potentially other class params, do not modify
                }
                ...
                img.dataset.originalSrcAttempted = 'true';
            } else {
                styleAvatarImageTag(img);
            }
        }
        avatarButton.dataset.processedBySize = size;
    }
    ...
    function applyCurrentAvatarSizeToAll(specificElements = null) {
        ...
        elementsToProcess.forEach(avatarButton => {
            if (avatarButton.dataset.processedBySize !== String(currentAvatarSize)) {
    ...
    function observeAvatars() {
        ...
        const callback = function(mutationsList, observer) {
            let newAvatarsFound = false;
            const addedNodesWithAvatars = [];
                    ...
                    mutation.addedNodes.forEach(node => {
                        if (node.nodeType === 1) {
                            if (node.matches && node.matches(ALL_AVATAR_BUTTON_SELECTORS)) {
                                addedNodesWithAvatars.push(node);
                                newAvatarsFound = true;
                            }
                            const childAvatars = node.querySelectorAll(ALL_AVATAR_BUTTON_SELECTORS);
                            if (childAvatars.length > 0) {
                                childAvatars.forEach(ca => addedNodesWithAvatars.push(ca));
                                newAvatarsFound = true;
                            }
                        }
                    });
            ...
            if (newAvatarsFound) {
                applyCurrentAvatarSizeToAll(addedNodesWithAvatars);
            }
        };
